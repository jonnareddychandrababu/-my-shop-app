...
const mantras: Mantra[] = [
  {
    ...
    meaning:
      "I bow to Lord Ganesha, the remover of obstacles. This is the most powerful seed (bija) mantra of Ganesha, chanted to invoke his blessings before any new beginning.",
  },
  {
    type: "Shloka",
    ...
    meaning:
      "O Lord with a curved trunk and mighty form, whose brilliance equals that of a million suns — please remove all obstacles from my path in all my endeavors, always.",
  },
  {
    type: "Gayatri Mantra",
    ...
    meaning:
      "We meditate on the one-tusked Lord with a curved trunk. May that elephant-faced one enlighten and inspire our intellect.",
  },
  {
    type: "Ashtanama",
    ...
    meaning:
      "O Ganapati, my Father, Morya! O auspicious form, Morya! A beloved devotional chant sung during Ganesh Chaturthi festivals, expressing deep love and surrender to Ganesha.",
  },
  {
    type: "Dhyana Mantra",
    ...
    meaning:
      "Salutations to Lord Ganesha. A simple yet profound mantra of reverence, chanted at the start of prayers, studies, or any auspicious activity to seek Ganesha's divine grace.",
  },
];
...
function renderGallery(): void {
  const track = document.getElementById("gallery-track") as HTMLDivElement;
  const dots = document.getElementById("gallery-dots") as HTMLDivElement;
  ...
  dots.innerHTML = images
    .map(
      (_, i) =>
        `<button class="dot ${i === 0 ? "active" : ""}" data-index="${i}" aria-label="Go to image ${i + 1}"></button>`
    )
    .join("");
...
function renderMantra(index: number): void {
  const mantra = mantras[index];
  if (!mantra) return;

  const card = document.getElementById("mantra-card") as HTMLDivElement;
  card.classList.add("fade-out");

  setTimeout(() => {
    const typeEl = document.getElementById("mantra-type") as HTMLSpanElement;
    const sanskritEl = document.getElementById("mantra-sanskrit") as HTMLParagraphElement;
    const translitEl = document.getElementById("mantra-transliteration") as HTMLParagraphElement;
    const meaningEl = document.getElementById("mantra-meaning") as HTMLParagraphElement;
    const counterEl = document.getElementById("mantra-counter") as HTMLSpanElement;
